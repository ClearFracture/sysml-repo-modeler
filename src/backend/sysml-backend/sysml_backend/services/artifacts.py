    ...
    def write_opencode_sysml(
        ...
        *,
        ...
        unresolved_services_path.write_text(
            _unresolved_summary("OpenCode Synthesis", validation, architecture),
            encoding="utf-8",
        )
        self._write_latest(
    ...
    def write_fallback_sysml(
        ...
        *,
        ...
        unresolved_services_path.write_text(
            ...
            + _architecture_unresolved_section(architecture),
            encoding="utf-8",
        )
        self._write_latest(
            suite_model_path, suite_evidence_path, unresolved_services_path, run_id
...
def _unresolved_summary(
    ...
    lines.append(_architecture_unresolved_section(architecture))
    return "\n".join(lines)


def _architecture_unresolved_section(
    architecture: dict[str, Any] | None,
) -> str:
    unresolved = (architecture or {}).get("unresolved", [])
    if not isinstance(unresolved, list) or not unresolved:
        return "\nArchitecture classification has no unresolved decisions.\n"
    lines = ["", "Unresolved architecture decisions:"]
    for item in unresolved:
        if not isinstance(item, dict):
            continue
        subject = item.get("subjectKey") or (
            f"{item.get('leftKey', 'unknown')} / {item.get('rightKey', 'unknown')}"
        )
        lines.append(f"- {item.get('kind', 'classification')}: {subject}")
    lines.append("")
    return "\n".join(lines)
