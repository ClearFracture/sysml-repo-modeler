...
def analysis_system_prompt() -> str:
    ...
    return "\n".join(
        [
            ...
            "- Model each repository and each external/third-party dependency as a top-level `part def`",
            ...
            "- Prioritize deployment manifests (Argo/Helm/k8s/compose), service and ingress definitions,",
            ...
            "- Capture every component, port, and connection the deployment/config evidence shows; you do",
            "  not need to read application source in this pass — depth and purpose come next.",
            "- Treat the pre-scan evidence checklist in the user message as required coverage: if it mentions",
            "  ingress, databases, messaging, identity/secrets, cloud platform, storage, observability, or",
            "  third-party APIs, those concepts should appear explicitly in the model when supported by the repos.",
    ...
    return "\n".join(
        [
            ...
            *[
            ...
            "- Keep every component, attribute, port, and connection from the architecture above unless a",
            "  repository plainly contradicts it; this pass adds detail, it does not remove parts.",
            "",
            "Documentation form:",
    ...
    return "\n".join(
        [
            ...
            *[f"- {diagnostic}" for diagnostic in diagnostics],
            ...
            "- Preserve the discovered architecture unless a diagnostic proves an element is malformed.",
            ...
            "- If validation reports missing coverage for an evidence category, add the corresponding component(s),",
            "  ports, attributes, or connections using the repository evidence.",
    ...
    return "\n".join(
        [
            ...
            *[
            ...
            "- Return one complete SysML v2 document only.",
            "- Preserve every existing component, port, attribute, connection, doc, and view unless it is plainly wrong.",
            "- Add missing evidence-backed architecture concepts as explicit top-level part definitions and MonitoredSuite instances.",
